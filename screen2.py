...
# TODO: fix bollinger bands. so+me stocks have distorted graphs (goog, gpro). write events to external file for
#       analysis by a separate program if desired, and/or to log total events for an entire portfolio.
# TODO: make event arrows look better. they're too imposing
...
def graphData(stock, MA1, MA2):
    ...
    try:
        ...
        api_url = 'http://chartapi.finance.yahoo.com/instrument/1.0/'+stock+'/chartdata;type=quote;range=1y/csv'
        stockFile = []
        try:
            sourceCode = urllib2.urlopen(api_url).read()
    ...
    try:
        ...
        candlestick(ax1, dataList[-SP:], width=.0006, colorup='#53c156', colordown='#ff1717')

        '''----------------------------------------------------------------------------------------------------'''
        def standard_deviation(tf,date,prices): # return an array of stdevs and correlating dates
            sd = []
            sddate = []
            x = tf
           ######
        ...
        bb_date, topBand, botBand, midBand = bollinger_bands(date, closep, 2, 10)
        ...
        ax1.plot(date[-SP:],closep[-SP:],'#F7FE2E',label=Label1, linewidth=1.0)
        ...
        for i in range(0, len(rsi)-1):
            if 30 <= rsi[i] <= 70:
                if (rsi[j] < 30):
                    appendLine = date[j], closep[j]
                    eventLine = str(date[j])+','+stock.upper()+','+str(closep[j])+',buy\n'
                    rsiTrigs.append(appendLine)
                    infile.write(eventLine)
                elif (rsi[j] > 70):
                    appendLine = date[j], closep[j]
                    eventLine = str(date[j])+','+stock.upper()+','+str(closep[j])+',sell\n'
                    rsiTrigs.append(appendLine)
                    infile.write(eventLine)
            i += 1
            j += 1
        ...
        if len(rsiTrigs) > 0:
            for event in rsiTrigs:
                ax1.annotate('EVENT', (event[0], event[1]), xytext = (0.5, 0.8), textcoords='axes fraction',
                             arrowprops=dict(facecolor='white',shrink=0.01), fontsize=7, color='w',
...
print "Welcome to the event profiler. Enter 'exit' to quit"
